...
import eu.kanade.tachiyomi.shizuku.IShellInterface
import eu.kanade.tachiyomi.shizuku.ShellInterface
...
class ShizukuInstaller(private val service: Service) : Installer(service) {
    ...
    private val shizukuArgs by lazy {
        Shizuku.UserServiceArgs(ComponentName(service, ShellInterface::class.java))
            .tag("shizuku_service")
    ...
    override fun onDestroy() {
        ...
        try {
            ...
            logcat(LogPriority.WARN, e) { "Failed to unregister receiver" }
        }
        scope.cancel()
        super.onDestroy()
    }
