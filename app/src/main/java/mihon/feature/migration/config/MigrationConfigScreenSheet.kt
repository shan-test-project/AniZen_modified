...
fun MigrationConfigScreenSheet(
    ...
    val migrationFlagsInt by preferences.migrationFlags().collectAsState()
    val migrationFlags = MigrationFlag.fromBit(migrationFlagsInt)
        ...
        Column(modifier = Modifier.fillMaxWidth()) {
            Column(
                    ...
                    MigrationFlag.entries
                        ...
                        .fastForEach { flag ->
                            ...
                            FilterChip(
                                selected = selected,
                                onClick = {
                                    val currentFlags = MigrationFlag.fromBit(preferences.migrationFlags().get())
                                    val newFlags = if (flag in currentFlags) {
                                        currentFlags - flag
                                    } else {
                                        currentFlags + flag
                                    }
                ...
                MigrationSheetSwitchItem(
                    ...
                    onClick = {
                        val currentFlags = MigrationFlag.fromBit(preferences.migrationFlags().get())
                        val newFlags = if (MigrationFlag.REMOVE_DOWNLOAD in currentFlags) {
                            currentFlags - MigrationFlag.REMOVE_DOWNLOAD
                        } else {
                            currentFlags + MigrationFlag.REMOVE_DOWNLOAD
                        }
                        preferences.migrationFlags().set(MigrationFlag.toBit(newFlags))
                ...
                if (fullSettings) {
                    ...
                    if (isSingleEntry) {
                        MigrationSheetSwitchItem(
                            title = stringResource(MR.strings.migrationConfigScreen_smartSearchSingleEntryTitle),
                            subtitle = stringResource(MR.strings.migrationConfigScreen_smartSearchSingleEntrySubtitle),
